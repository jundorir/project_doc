...
Now that you have created your New-Swarm wallet and your Bee node has
begun to participate in the global New-Swarm network, we can use the Debug
...
```bash
bee start --debug-api-enable --debug-api-addr=localhost:11635
```
...
```bash
curl -s http://localhost:11635/peers | jq '.peers | length'
```
...
```bash
curl -X GET http://localhost:11635/topology | jq
```
